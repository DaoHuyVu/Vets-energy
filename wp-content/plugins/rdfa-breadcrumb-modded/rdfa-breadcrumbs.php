<?php
/**
 * Plugin Name: RDFa Breadcrumb (Modded by Amitech)
 * Plugin URI: https://wordpress.org/plugins/rdfa-breadcrumb-modded/
 ...
 * @Version 2.3
 * @author Nitin Yawalkar <mhyawalkar@gmail.com>
 * @copyright Copyright (c) 2011-2012, Nitin Yawalkar
 ...
 */
 ...
 function rdfa_breadcrumb() {
	...
	$args = array(
		...
		'home_title' 		=> $opts['home_title'] ? $opts['home_title'] : __( 'Home', 'rdfa-breadcrumb' ),
	...
	$breadcrumbs = '<!-- RDFa Breadcrumbs Plugin by Nitin Yawalkar (Modded by Amitech) -->';
	$breadcrumbs .= '<div class="breadcrumb breadcrumbs"><div class="rdfa-breadcrumb"><div itemscope itemtype="https://schema.org/BreadcrumbList">';
...
function rdfa_breadcrumb_get_items( $args ) {
	...
	/*if ( !is_front_page() ) {
		...
		$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="'. $home_url .'" href="'. $home_url .'" class="home"><span itemprop="name">' . $args['home_title'] . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
	}*/
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' === $wp_query->post->post_type && $args['show_blog'] ) {
			$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . get_permalink( get_option( 'page_for_posts' ) ) . '" href="' . get_permalink( get_option( 'page_for_posts' ) ) . '"><span itemprop="name">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
		}
		...
		if ( 'page' !== $wp_query->post->post_type ) {
			...
			if ( function_exists( 'get_post_type_archive_link' ) && !empty( $post_type_object->has_archive ) )
				$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . get_post_type_archive_link( $post_type ) . '" href="' . get_post_type_archive_link( $post_type ) . '" title="' . esc_attr( $post_type_object->labels->name ) . '"><span itemprop="name">' . $post_type_object->labels->name . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
			...
			if ( isset( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) && is_taxonomy_hierarchical( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) ) {
				...
				$item = array_merge( $item, rdfa_breadcrumb_get_term_parents( $terms[0], $args["singular_{$wp_query->post->post_type}_taxonomy"], $args['separator'], $count ) );
			}
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			if ( ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent ) && $parents = rdfa_breadcrumb_get_term_parents( $term->parent, $term->taxonomy, $args['separator'], $count ) )
				$item = array_merge( $item, $parents );
	...
	$item['last'] = '<span itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem"><span itemprop="name">' . $item['last'] . '</span><meta itemprop="position" content="' . $count . '" /></span>';
...
function rdfa_breadcrumb_get_bbpress_items( $args = array() ) {
	...
	if ( !empty( $post_type_object->has_archive ) && !bbp_is_forum_archive() )
		$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '" href="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '"><span itemprop="name">' . bbp_get_forum_archive_title() . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
			$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . bbp_get_topic_permalink( $topic_id ) . '" href="' . bbp_get_topic_permalink( $topic_id ) . '"><span itemprop="name">' . bbp_get_topic_title( $topic_id ) . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
		else
	...
	elseif ( bbp_is_single_reply() ) {
		...
		if ( !bbp_is_reply_edit() ) {
			...
			$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . bbp_get_reply_url( $reply_id ) . '" href="' . bbp_get_reply_url( $reply_id ) . '"><span itemprop="name">' . bbp_get_reply_title( $reply_id ) . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
			$item[] = __( 'Edit', 'rdfa-breadcrumb' );
		}
	...
	elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
		...
		if ( bbp_is_single_user_edit() ) {
			$item[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . bbp_get_user_profile_url() . '" href="' . bbp_get_user_profile_url() . '"><span itemprop="name">' . bbp_get_displayed_user_field( 'display_name' ) . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
			$item[] = __( 'Edit' );
		} else {
...
function rdfa_breadcrumb_get_parents( $post_id = '', $separator = '/' ) {
	...
	while ( $post_id ) {
		...
		$parents[]  = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . get_permalink( $post_id ) . '" href="' . get_permalink( $post_id ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '"><span itemprop="name">' . get_the_title( $post_id ) . '</span></a><meta itemprop="position" content="' . $count++ .'" /></span>';
 ...
 */
function rdfa_breadcrumb_get_term_parents( $parent_id = '', $taxonomy = '', $separator = '/', &$count ) {
	...
	while ( $parent_id ) {
		...
		$parents[] = '<span itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a itemscope itemtype="https://schema.org/WebPage" itemprop="item" itemid="' . get_term_link( $parent, $taxonomy ) . '" href="' . get_term_link( $parent, $taxonomy ) . '" title="' . esc_attr( $parent->name ) . '"><span itemprop="name">' . $parent->name . '</span>';
	...
	for ( $i = 0; $i < count( $parents ); $i ++ ) 
		$parents[$i] .= '</a><meta itemprop="position" content="' . $count++ .'" /></span>';
...
function rdfa_breadcrumb_settings() {
	...
	<div style="width:30%; float:right;">
		...
		<div id="accordion4">
			<h3><a href="#">Other Links</a></h3>
			<div>
				<ul style="list-style: square;">
					<li><a href="https://www/nitinyawalkar.com" style="text-decoration:none;">My Blog</a></li>
